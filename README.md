...
## News 
* ```2026.3.19 ``` 🚀 We present [dots.mocr](https://huggingface.co/rednote-hilab/dots.mocr). Beyond achieving state-of-the-art (SOTA) performance in standard multilingual document parsing among models of comparable size, **dots.mocr** excels at converting structured graphics (e.g., charts, UI layouts, scientific figures and etc.) directly into SVG code. Additionally, the model retains comparable performance in general vision tasks to Qwen3-VL-4B. Simultaneously, we are releasing [dots.mocr-svg](https://huggingface.co/rednote-hilab/dots.mocr-svg), a variant specifically optimized for robust image-to-SVG parsing tasks. More information can be found [in the paper](https://arxiv.org/abs/2603.13032v1).
